...
export interface YjsTestEditor extends Editor {
  isRemote: boolean;
  doc: Y.Doc;
  syncDoc: SyncDoc;
...
export const YjsTestEditor = {
  /**
   * Apply slate ops to Yjs
   */
  applySlateOpsToYjs: (e: YjsTestEditor, operations: Operation[]) => {
    e.doc.transact(() => {
   ...
   */
  applyYjsEventsToSlate: (e: YjsTestEditor, events: Y.YEvent[]) => {
   ...
   */
  captureYjsUpdate: (e: YjsTestEditor, update: Uint8Array, _origin: any) => {
   ...
   */
  getCapturedYjsUpdates: (e: YjsTestEditor): Uint8Array[] => {
   ...
   */
  applyYjsUpdatesToYjs: async (e: YjsTestEditor, updates: Uint8Array[]) => {
    await Promise.all(
      updates.map((update) => {
        YjsTestEditor.applyYjsUpdateToYjs(e, update);
      })
    );
  },
   ...
   */
  applySlateOpToSlate: (e: YjsTestEditor, op: Operation): Promise<void> => {
    return new Promise((resolve) => {
      e.onChangeComplete = () => {
        e.onChangeComplete = () => void {};
        resolve();
   ...
   */
  applySlateOpsToSlate: async (e: YjsTestEditor, operations: Operation[]) => {
    await Promise.all(
      operations.map((op) => {
        YjsTestEditor.applySlateOpToSlate(e, op);
      })
    );
  },
};

export const withYjsTest = <T extends Editor>(editor: T): T & YjsTestEditor => {
  const e = editor as T & YjsTestEditor;

  const doc = new Y.Doc();
  const syncDoc = doc.getArray<SyncElement>('content');

  doc.on('update', (updateMessage: Uint8Array, origin: any) => {
    YjsTestEditor.captureYjsUpdate(e, updateMessage, origin);
  });
  syncDoc.observeDeep((events) => {
    YjsTestEditor.applyYjsEventsToSlate(e, events);
  });
  ...
  e.onChange = () => {
    if (!e.isRemote) {
      YjsTestEditor.applySlateOpsToYjs(e, e.operations);
    }
