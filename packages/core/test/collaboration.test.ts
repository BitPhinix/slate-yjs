...
import { yTextToSlateElement } from '../src/utils/convert';
...
describe('slate operations propagate between editors', () => {
  tests.forEach(([testName, input, ...cases]) => {
    it(`${testName}`, async () => {
      ...
      expect(yTextToSlateElement(src.sharedRoot).children).toEqual(input);
      expect(yTextToSlateElement(dst.sharedRoot).children).toEqual(input);
      ...
      while (toTest.length > 0) {
        ...
        expect(yTextToSlateElement(src.sharedRoot).children).toEqual(output);
        expect(yTextToSlateElement(dst.sharedRoot).children).toEqual(output);
